  ...
  return (
    ...
    >
      ...
      >
        ...
        <p id={titleId} className="px-5.5 pb-2.5 text-kicker uppercase tracking-wider text-ink-3">
          {title}
        </p>
        ...
        <ul>
          {normalItems.map((item) => (
            <li key={item.label}>
              <button type="button" onClick={item.onClick} className={ACTION_CLASS}>
          ...
          {dangerItems.map((item) => (
            <li key={item.label}>
              ...
              >
                ...
                <span
                  aria-hidden
                  className="flex size-9 shrink-0 items-center justify-center rounded-nav-item bg-overdue-soft text-overdue"
                >
                  {item.icon}
                </span>
                {item.label}
