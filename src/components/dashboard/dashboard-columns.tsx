...
export const dashboardColumns: ColumnDef<Invoice>[] = [
  ...
  },
  {
    accessorKey: "amount",
    header: "Amount",
    sortingFn: (a, b) => parseRupeeString(a.original.amount) - parseRupeeString(b.original.amount),
    cell: ({ getValue }) => (
  ...
  },
  {
    ...
    enableSorting: false,
    cell: ({ row }) => {
      const href = row.original.invoiceUuid
        ? `/invoices/${row.original.invoiceUuid}`
        : `/invoices/${row.original.id.replace("#", "")}`;
      return (
        <Link
          href={href}
