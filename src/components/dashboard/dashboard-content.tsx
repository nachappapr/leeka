...
export async function DashboardContent() {
  // business_id is resolved + verified by the proxy and forwarded as a trusted
  // header, so the cached data functions receive it without an extra DB lookup.
  const businessId = (await headers()).get("x-business-id");

  const [summary, invoices, isProPlan, hasCustomers] = businessId
    ? await Promise.all([
        getDashboardSummary({ businessId }),
        getRecentInvoices({ businessId }),
        isPro(businessId),
        businessHasCustomers({ businessId }),
      ])
    : [ZERO_DASHBOARD_SUMMARY, [] as ReadonlyArray<Invoice>, false, false];
