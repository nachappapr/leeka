...
export function MarkPaidModal({
  ...
  function handleConfirm() {
    if (isPending) return;
    if (!invoiceUuid) {
      brandToast.error({ title: "Couldn't mark as paid" });
      return;
    }
    startTransition(async () => {
      const result = await markInvoicePaid({ invoiceId: invoiceUuid });
      if (!result.ok) {
        brandToast.error({ title: "Couldn't mark as paid", sub: result.error });
        return;
      }
      brandToast.success({ title: "Marked as paid", sub: `${invoice.id} is now paid` });
      onOpenChange(false);
    });
  }
  ...
  return (
    <Modal open={open} onOpenChange={onOpenChange}>
      <ModalContent initialFocus={cancelRef} finalFocus={finalFocus}>
        <ModalHeader>
          <div className="flex size-11 shrink-0 items-center justify-center rounded-md bg-paid">
            <Check className="size-5.5 text-card" aria-hidden />
          </div>
          <div className="flex min-w-0 flex-1 flex-col">
            <ModalTitle>Mark this invoice as paid?</ModalTitle>
            <ModalDescription>
              This records the full amount as received and moves {invoice.id} to{" "}
              <strong className="font-extrabold text-ink">Paid</strong>. You can change it back
              later.
            </ModalDescription>
          </div>
        ...
        <ModalFooter className="max-mobile:flex-wrap justify-end">
          ...
          <p role="status" aria-live="polite" aria-atomic="true" className="sr-only">
            {isPending ? "Marking invoice as paid…" : ""}
          </p>
        </ModalFooter>
      </ModalContent>
    </Modal>
