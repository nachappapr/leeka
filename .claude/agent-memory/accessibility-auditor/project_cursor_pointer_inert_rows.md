...
Both `DataRow` (`src/components/ui/custom/data-table.tsx:65`) and `DataListRow` (`src/components/ui/custom/data-list-row.tsx:12`) apply `cursor-pointer hover:bg-coral/5` but have no `onClick`, `tabIndex`, or keyboard handlers. Interactivity is flagged as a FOLLOW-UP in the design pipeline; the rows are intentionally inert for now.
